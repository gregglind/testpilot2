...
let attemptFlight = function(spec){
  ...
  if (!d){
    ...
    known[name] = d = {spec:spec, status:'running',firstseen: Date.now(), results: {run:[],cleanup:[]}}
  }
  ...
  if (airborne[name])
    ...
    if (has(['complete','stopped'],known[name].status)){
      tplog("airborne:allow-stay-unrun", name, known[name].status );
    } else {
      tplog("airborne:starting:",name);
      airborne[name] = Flight(d);
      airborne[name].attempt().then(delete airborne[name], ohno);
    }
  }
  return promise
}